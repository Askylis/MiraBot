...
namespace MiraBot.Modules
{
    ...
    public class GroceryAssistantModule : InteractionModuleBase<SocketInteractionContext>
    {
        ...
        private readonly ILogger<GroceryAssistantModule> _logger;
        ...
        public GroceryAssistantModule(GroceryAssistant groceryAssistant, InteractiveService interactiveService,
            ModuleHelpers moduleHelpers, UserCommunications comms, ILogger<GroceryAssistantModule> logger)
        {
        ...
        public async Task AddMealAsync()
        {
            ...
            {
                _logger.LogCritical(ex, "Error adding new meal from {username}", owner.UserName);
                await ReplyAsync("Something went wrong while adding this meal. Please try again!");
                return;
            }
            _logger.LogInformation("New meal added by {username}", owner.UserName);
        ...
        public async Task DeleteMealAsync()
        {
            ...
            while (index != -1)
            {
                ...
                {
                    _logger.LogCritical(ex, "Error while deleting meal from {username}", owner.UserName);
                    await ReplyAsync("Something went wrong while deleting this meal. Please try again!");
                    return;
                }
                _logger.LogInformation("Meal deleted by {username}", owner.UserName);
        ...
        public async Task GenerateMealsListAsync()
        {
            ...
            _logger.LogInformation("New GA list generated by {username} containing {itemCount} items.", owner.UserName, selectedMeals.Count);
        ...
        public async Task ConvertMealsFileAsync()
        {
            ...
            _logger.LogInformation("{username} converted an old GA file. {itemCount} items were converted from this file.", owner.UserName, meals.Count);
        }
        ...
        public async Task AddRecipeAsync(bool isEdit, int mealId = 0)
        {
            ...
            {
                _logger.LogCritical(ex, "Error adding recipe to {mealName} from {username}", meal.Name, owner.UserName);
                await ReplyAsync("Something went wrong while adding this recipe to your meal. Please try again!");
                return;
            }
        ...
        public async Task ShareRecipeAsync(string recipientName)
        {
            ...
            _logger.LogInformation("Recipe sent to {username} from {username}.", recipient.UserName, owner.UserName);
